...
void main()
{
    ...
    vec3 ambient = ambientt * vec3(1.0, 1.0, 1.0);
	
    float intcs = max(dot(normal, lightdir), 0.0);
	
	
    vec3 diffuse = intcs * vec3(1.0, 1.0, 1.0);
	
	
    vec3 temp = normalize(-fragPos);
	
    vec3 mirror = reflect(-lightdir, normal);
	
    float intfinal = pow(max(dot(temp, mirror), 0.0), 8.0);
	
	
    vec3 spectra = intfinal * vec3(1.0, 1.0, 1.0);
	
    vec3 summedvers = ambient + diffuse + spectra;
	
    vec3 sonrgb = summedvers * texture2D(tex, vTexCoord).rgb;
	
    if(isLightSource)
	{
		
        gl_FragColor = vec4(texture2D(tex, vTexCoord).rgb, 1.0);
   
   }
   else
   {
		
        gl_FragColor = vec4(sonrgb, 1.0); 
    }
}
`;
